...
var getCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		responseJSON, err := json.MarshalIndent(string(pingResponse.Msg.Data), "", " ")
		if err != nil {
...
var deleteCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		pingResponse, err := client.Delete(context.Background(), connect.NewRequest(&v1.DeleteRequest{
			ObjectKey: args[0],
		}))
		if err != nil {
			return fmt.Errorf("failed to get item with key '%s': %w", args[0], err)
...
func init() {
	...
	storeCmd.PersistentFlags().StringVar(&address, "address", "http://127.0.0.1:8084", "Set the address of the storage service")
